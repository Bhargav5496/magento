...
Product.Gallery.prototype = {
    ...
    initialize: function (containerId, imageTypes) {
        ...
        this.template = new Template(
            '<tr id="__id__" class="preview">' +
                this.getElement("template").innerHTML +
                "</tr>",
            new RegExp("(^|.|\\r|\\n)(__([a-zA-Z0-9_]+)__)", "")
        );
        this.fixParentTable();
    ...
    onImageTabMove: function (event) {
        ...
        if (
            ...
            this.container.select('input[type="radio"]').each(
                function (radio) {
    ...
    updateImage: function (file) {
        ...
        this.images[index].label = this.getFileElement(
            file,
            "cell-label input"
        ).value;
        this.images[index].position = this.getFileElement(
            file,
            "cell-position input"
        ).value;
        this.images[index].removed = this.getFileElement(
            file,
            "cell-remove input"
        ).checked
            ? 1
            : 0;
        this.images[index].disabled = this.getFileElement(
            file,
            "cell-disable input"
        ).checked
            ? 1
    ...
    setProductImages: function (file) {
        $H(this.imageTypes).each(
            function (pair) {
                if (
                    this.getFileElement(file, "cell-" + pair.key + " input")
                        .checked
                ) {
                    this.imagesValues[pair.key] =
                        file == "no_selection" ? null : file;
                }
            }.bind(this)
        );

        this.getElement("save_image").value = Object.toJSON(
            $H(this.imagesValues)
        );
    },
    updateVisualisation: function (file) {
        ...
        this.getFileElement(file, "cell-remove input").checked =
            image.removed == 1;
        this.getFileElement(file, "cell-disable input").checked =
            image.disabled == 1;
        $H(this.imageTypes).each(
            function (pair) {
    ...
    updateUseDefault: function () {
        if (this.getElement("default")) {
            this.getElement("default")
                .select("input")
                .each(
                    function (input) {
                        $(this.containerId)
                            .select(".cell-" + input.value + " input")
                            .each(function (radio) {
...
Product.Attributes.prototype = {
    ...
    addRow: function (html) {
        var attributesContainer = $$(
            "#group_fields" + this.getConfig().group_id + " .form-list tbody"
        )[0];
        ...
        var element = childs[childs.size() - 1].select(
            "input",
            "select",
            "textarea"
        )[0];
        if (element) {
            window.scrollTo(
                0,
                Position.cumulativeOffset(element)[1] + element.offsetHeight
            );
        }
    },
};
...
Product.Configurable.prototype = {
    initialize: function (attributes, links, idPrefix, grid, readonly) {
        ...
        this.addAttributeTemplate = new Template(
            $(idPrefix + "attribute_template")
                .innerHTML.replace(/__id__/g, "'{{html_id}}'")
                .replace(/ template no-display/g, ""),
            this.templatesSyntax
        );
        this.addValueTemplate = new Template(
            $(idPrefix + "value_template")
                .innerHTML.replace(/__id__/g, "'{{html_id}}'")
                .replace(/ template no-display/g, ""),
            this.templatesSyntax
        );
        this.pricingValueTemplate = new Template(
            $(idPrefix + "simple_pricing").innerHTML,
            this.templatesSyntax
        );
        ...
        // attribute
        // label
        this.onValuePriceUpdate =
            this.updateValuePrice.bindAsEventListener(this); // Update pricing value
        this.onValueTypeUpdate = this.updateValueType.bindAsEventListener(this); // Update
        // pricing
        // type
        this.onValueDefaultUpdate =
            this.updateValueUseDefault.bindAsEventListener(this);
        ...
        this.attributes.each(
            function (attribute, index) {
                ...
                template = template.replace(
                    new RegExp(' readonly="label"', "ig"),
                    label_readonly
                );
                template = template.replace(
                    new RegExp(' checked="use_default"', "ig"),
                    use_default_checked
                );
                li.update(template);
                ...
                Event.observe(
                    li.down(".attribute-use-default-label"),
                    "change",
                    this.onLabelUpdate
                );
            }.bind(this)
        ...
        this.links.each(function (pair) {
            for (var i = 0, length = pair.value.length; i < length; i++) {
                ...
                uniqueAttributeValues
                    ...
                    .set(attribute.value_index, attribute);
            }
        });
        /* Updating attributes value container */
        this.container.childElements().each(
            function (row) {
                var attribute = row.attributeObject;
                for (
                    var i = 0, length = attribute.values.length;
                    i < length;
                    i++
                ) {
                    if (
                        ...
                        uniqueAttributeValues
                            .get(attribute.attribute_id)
                            .keys()
                            .indexOf(attribute.values[i].value_index) == -1
                    ) {
                        row.attributeValues
                            .childElements()
                            .each(function (elem) {
                                if (
                                    elem.valueObject.value_index ==
                        ...
                        uniqueAttributeValues
                            .get(attribute.attribute_id)
                            .unset(attribute.values[i].value_index);
                    }
                }
    ...
    createValueRow: function (container, value) {
        ...
        templateVariables.set(
            "html_id",
            container.id + "_" + this.valueAutoIndex
        );
        ...
        if (
            priceTypeField != undefined &&
            priceTypeField.options != undefined
        ) {
            if (parseInt(value.is_percent)) {
                priceTypeField.options[1].selected =
    ...
    updateValuePrice: function (event) {
        var li = Event.findElement(event, "LI");
        li.valueObject.pricing_value = Event.element(event).value.blank()
            ? null
    ...
    updateValueType: function (event) {
        var li = Event.findElement(event, "LI");
        li.valueObject.is_percent = Event.element(event).value.blank()
            ? null
            : Event.element(event).value;
        this.updateSimpleForm();
        this.updateSaveInput();
    ...
    updateSaveInput: function () {
        $(this.idPrefix + "save_attributes").value = Object.toJSON(
            this.attributes
        );
    ...
    quickCreateNewProduct: function () {
        ...
        var validationResult = $(this.idPrefix + "simple_form")
        ...
        var params = Form.serializeElements(
            $(this.idPrefix + "simple_form").select(
                "input",
                "select",
                "textarea"
            ),
    ...
    quickCreateNewProductComplete: function (transport) {
        ...
        if (result.error) {
            if (result.error.fields) {
                ...
                $(this.idPrefix + "simple_form").addClassName(
                    "ignore-validate"
                );
            } else {
        ...
        result.attributes.each(
            function (attribute) {
                var attr = this.getAttributeById(attribute.attribute_id);
                if (
                    !this.getValueByIndex(attr, attribute.value_index) &&
                    result.pricing &&
                    result.pricing[attr.attribute_code]
                ) {
                    attribute.is_percent =
                        result.pricing[attr.attribute_code].is_percent;
                    attribute.pricing_value =
                        result.pricing[attr.attribute_code].value == null
                            ? ""
        ...
        this.attributes.each(
            function (attribute) {
                attributes.push({
                    ...
                    value_index: $("simple_product_" + attribute.attribute_code)
                        .value,
                });
            }.bind(this)
        );
    ...
    showPricing: function (select, attributeCode) {
        ...
        if (
            ...
            Element.insert(select, {
                after:
                    '<div class="left"></div> <div id="simple_product_' +
                    attributeCode +
                    '_pricing_container" class="left"></div>',
            });
            var newContainer = select.next("div");
            select.parentNode.removeChild(select);
            newContainer.appendChild(select);
            // Fix visualization bug
            $(this.idPrefix + "simple_form").down(".form-list").style.width =
                "100%";
        }
        ...
        if (select.value) {
            ...
            if (!value) {
                if (!container.down(".attribute-price")) {
                    ...
                    var priceTypeField = container.down(
                ...
                container.update(
                    this.pricingValueViewTemplate.evaluate({
                        value:
                            (parseFloat(value.pricing_value) > 0 ? "+" : "") +
                            parseFloat(value.pricing_value) +
                            (parseInt(value.is_percent) > 0 ? "%" : ""),
                    })
                );
                $("simple_product_" + attributeCode + "_pricing_value").value =
                    value.pricing_value;
                $("simple_product_" + attributeCode + "_pricing_type").value =
                    value.is_percent;
            } else {
                container.update("");
                $("simple_product_" + attributeCode + "_pricing_value").value =
    ...
    updateSimplePricing: function (evt) {
        ...
        if (!element.priceField.value.blank()) {
            $(
                "simple_product_" + element.attributeCode + "_pricing_value"
            ).value = element.priceField.value;
            $(
                "simple_product_" + element.attributeCode + "_pricing_type"
            ).value = element.typeField.value;
        } else {
            $(
                "simple_product_" + element.attributeCode + "_pricing_value"
            ).value = null;
