...
class Bhargav_Eavmgmt_ImportController extends Mage_Adminhtml_Controller_Action
{
                    ...
                    else
                    {
                        ...
                        if (!$existingOption) {
                            ...
                            if(!$connection->insert($tableName, $data)){
                                throw new Exception($data['value']." option is not inserted.", 1);
                            }
                            Mage::getSingleton('core/session')->addSuccess($count . ' Options added.');
                            $this->_redirect('*/adminhtml_eavmgmt/index');
                        }
                    }
                }
                $total = count($csvData) - 1 - $count;
                Mage::getSingleton('core/session')->addSuccess($total . ' Options already exists.');
            }
        } catch (Exception $e) {
